...
async function runSecurityAuditTests() {
    ...
    try {
        ...
        reportResult("Test 1: Anonymous user cannot read profiles table", passed, passed ? "Access denied as expected by default-deny RLS." : `Leaked ${data?.length} profile rows to unauthenticated client!`);
    }
    catch (e) {
    ...
    try {
        ...
        const passed = data === null || (Array.isArray(data) && data.length === 0);
        reportResult("Test 2: Anonymous user cannot read audit_logs table", passed, passed ? "Access denied as expected by default-deny RLS." : `Leaked ${data?.length} audit log rows to unauthenticated client!`);
    }
    catch (e) {
    ...
    try {
        ...
        const passed = data === null || (Array.isArray(data) && data.length === 0);
        reportResult("Test 3: Anonymous user cannot read coupon_usage table", passed, passed ? "Access denied as expected by default-deny RLS." : `Leaked ${data?.length} coupon usage rows!`);
    }
    catch (e) {
    ...
    try {
        ...
        reportResult("Test 4: Privilege Escalation — Anonymous cannot insert Owner profile", passed, passed ? `Rejected with error: ${error?.message}` : "CRITICAL SECURITY BREACH: Anonymous user created Owner profile!");
    }
    catch (e) {
    ...
    try {
        ...
        const passed = error !== null;
        reportResult("Test 5: Privilege Escalation — Anonymous cannot create coupons", passed, passed ? `Rejected with error: ${error?.message}` : "SECURITY BREACH: Anonymous client inserted arbitrary coupon!");
    }
    catch (e) {
